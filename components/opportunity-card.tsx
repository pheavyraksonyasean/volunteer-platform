...
import { MapPin, Clock, Calendar, Users, Heart, Share2, Bookmark, Eye } from "lucide-react"
...
export default function OpportunityCard({ opportunity, onApply, showFullDescription = false }: OpportunityCardProps) {
        ...
        <CardContent className="p-0">
          <div className="relative">
            ...
            <div className="absolute top-3 left-3 flex gap-2">
              ...
              <Badge className="bg-white text-gray-900 hover:bg-gray-100">{opportunity.category}</Badge>
            </div>
            <div className="absolute top-3 right-3 flex gap-2">
              ...
              >
                <Bookmark className={`h-4 w-4 ${isBookmarked ? "fill-blue-500 text-blue-500" : "text-gray-600"}`} />
              </Button>
            </div>
            {isAlmostFull && !isFull && (
          ...
          <div className="p-6">
            <div className="flex justify-between items-start mb-2">
              <h3 className="text-xl font-semibold text-gray-900 line-clamp-1">{opportunity.title}</h3>
              <Badge variant="outline" className="ml-2 shrink-0">
                {opportunity.volunteers}/{opportunity.maxVolunteers}
              </Badge>
            </div>

            <p className="text-indigo-600 font-medium mb-3">{opportunity.organization}</p>
            ...
            <div className="flex gap-2">
              <Button className="flex-1" onClick={handleApply} disabled={isFull}>
                {isFull ? "Full" : "Apply Now"}
              </Button>
              <Button variant="outline" size="sm" className="px-3 bg-transparent">
                <Eye className="h-4 w-4" />
              </Button>
              <Button variant="outline" size="sm" className="px-3 bg-transparent" onClick={handleShare}>
